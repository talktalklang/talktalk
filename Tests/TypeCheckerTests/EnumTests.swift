...
struct EnumTests: TypeCheckerTest {
	...
	@Test("Can infer a generic enum type") func generics() throws {
		...
		#expect(enumType.cases[0].attachedTypes.count == 1)
		#expect(enumType.cases[0].attachedTypes[0] == .typeVar("Wrapped", 89)) // Make sure int doesn't leak to outer generic
		...
		#expect(enumType.cases[0].attachedTypes[0] == .typeVar("Wrapped", 89)) // Make sure we're still good here

...
struct SyntaxTreeTests {
	func parse<T: Syntax, R: Syntax>(
		...
		let root = try! SyntaxTree.parse(source: .init(path: "parser", source: string)).decls[0] as! R
	...
	func parse<T: Syntax>(
		...
		let root = try! SyntaxTree.parse(source: .init(path: "parser", source: string)).decls[0]
		return root as! T
	}

	func parseError(_ string: String) -> [TalkTalkSyntax.Error] {
		do {
			_ = try SyntaxTree.parse(source: .init(path: "parser", source: string)).decls[0]
	...
	@Test("Can get node from position") func nodeAt() throws {
		...
		let tree = try SyntaxTree.parse(source: .init(path: "parser", source: source))

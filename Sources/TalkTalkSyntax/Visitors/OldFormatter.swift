...
public struct OldFormatter: Visitor {
	...
	public func visit(_ expr: MemberExprSyntax, _ context: Context) throws -> String {
		...
		if let receiver = expr.receiver {
			result += try receiver.accept(self, context)
		}
		result += "."
	...
	public func visit(_ expr: Argument, _ context: Context) throws -> String {
	...
	public func visit(_: FuncSignatureDeclSyntax, _: Context) throws -> String {
		"(typeName)"
	}

	public func visit(_ expr: EnumDeclSyntax, _ context: Context) throws -> String {
		var result = "enum \(expr.nameToken.lexeme) "
		result += try expr.body.accept(self, context)
		result += "}"
	...
	public func visit(_ expr: EnumCaseDeclSyntax, _ context: Context) throws -> String {
		var result = "case \(expr.nameToken.lexeme)"
		if !expr.attachedTypes.isEmpty {
			result += "("
			result += try expr.attachedTypes.map { try $0.accept(self, context) }.joined(separator: ", ")
			result += ")"
		}
		return result
	}

	public func visit(_ expr: MatchStatementSyntax, _ context: Context) throws -> String {
		var result = "match "
		result += try expr.target.accept(self, context)
		result += "{\n"
		for kase in expr.cases {
			result += try kase.accept(self, context)
		}
		result += "}"
		return result
	}
		...
		if !expr.body.isEmpty {
			...
			result += try indenting {
				...
				for stmt in expr.body {
					try inner.append(stmt.accept($0, context))
				}
	...
	public func visit(_: EnumMemberExprSyntax, _: Context) throws -> String {
		#warning("Generated by Dev/generate-type.rb")
		return "(typeName)"
	}

	public func visit(_: InterpolatedStringExprSyntax, _: Context) throws -> String {
		#warning("Generated by Dev/generate-type.rb")
		return "(typeName)"
	}

	public func visit(_ expr: ForStmtSyntax, _ context: Context) throws -> String {
		#warning("Generated by Dev/generate-type.rb")
		return "(typeName)"
	}

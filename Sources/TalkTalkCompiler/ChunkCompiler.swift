...
public class ChunkCompiler: AnalyzedVisitor {
	...
	public func visit(_ expr: AnalyzedDefExpr, _ chunk: Chunk) throws {
		...
		chunk.emit(.symbol(variable.symbol), line: expr.location.line)
...
//			chunk.emit(byte: variable.slot, line: parameter.location.line)
	...
	public func visit(_ expr: AnalyzedFuncExpr, _ chunk: Chunk) throws {
		...
		for parameter in expr.analyzedParams.paramsAnalyzed {
			...
			functionChunk.emit(.symbol(variable.symbol), line: parameter.location.line)
		}
	...
	public func visit(_ expr: AnalyzedVarDecl, _ chunk: Chunk) throws {
		if expr.environment.isModuleScope {
			...
			if let value = expr.valueAnalyzed {
				...
				chunk.emit(.symbol(variable.symbol), line: value.location.line)
			}
...
//		chunk.emit(byte: Byte(expr.elements.count), line: expr.location.line)
	...
	func resolveVariable(named varName: String, symbol: Symbol?, chunk: Chunk) -> Variable? {
		...
		if let symbol = resolveLocal(named: varName) {
			return Variable(
				name: varName,
				symbol: symbol,
				depth: scopeDepth,
...
//		}
		...
		if case let .struct(name) = symbol?.kind {
			return Variable(
				name: varName,
				symbol: .struct(module.name, name),
				depth: scopeDepth,
				isCaptured: false,
				getter: .getStruct,
				setter: .setStruct
		...
		if let fn = BuiltinFunction.list.first(
			...
			return Variable(
				name: varName,
				symbol: .function("[builtin]", varName, fn.parameters),
				depth: scopeDepth,
				isCaptured: false,
				getter: .getBuiltin,
				setter: .setBuiltin
	...
	public func resolveLocal(named name: String) -> Symbol? {
...
//	}
	...
	private func emitLazyInitializer(for expr: any AnalyzedVarLetDecl, in chunk: Chunk) throws -> Variable {
		...
		initializerChunk.emit(.symbol(variable.symbol), line: expr.location.line)
...
//	}
	...
	private func synthesizeInit(for structType: StructType) -> Chunk {
		...
		for (variable, property) in variables {
			...
			chunk.emit(.symbol(variable.symbol), line: 9999)
